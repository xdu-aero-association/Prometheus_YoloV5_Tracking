...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>回 调 函 数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void switch_cb(const mavros_msgs::Thrust::ConstPtr & msg)
{
    Switch = *msg;
    cal = 0;
    cal = Switch.thrust;
...
int main(int argc, char **argv)
{
    ...
    //【订阅】
    ros::Subscriber aruco_sub = nh.subscribe<mavros_msgs::Thrust>("/prometheus/switch", 50, switch_cb);

    //【订阅】无人机状态
    ros::Subscriber drone_state_sub = nh.subscribe<prometheus_msgs::DroneState>("/prometheus/drone_state", 1, drone_state_cb);
    ...
    while(ros::ok())
    {
        ...
        if (fabs(gimbal_att_deg[2])< 15)
        {
            state_desired[3] = 0;
            if(gimbal_att_deg[1] < 30)
            {
                state_desired[0] = PID_realize(&pid1,35,gimbal_att_deg[1]);
            }
            else
            {
                pid1.integral = 0;
        ...
        {
            state_desired[0] = 0;
            state_desired[3]  = - PID_realize(&pid2, 0, gimbal_att_deg[2]);
        }
        if(cal == 1)
        {
            state_desired[0] = 0;
    ...
    //state_desired[0] = 0;
    cout << "gimbal_att_sp : " << gimbal_att_deg[0] << " [deg] "<< gimbal_att_deg[1] << " [deg] "<< gimbal_att_deg[2] << " [deg] "<<endl;
    cout << "state_desired : " << state_desired[0] << " [deg] "<< state_desired[1] << " [deg] "<< state_desired[2] << " [deg] " << state_desired[3] << " [deg] " << endl;        
